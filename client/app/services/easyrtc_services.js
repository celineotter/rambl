...
.factory('EasyRTC', ['$window', '$timeout', '$location',
  function ($window, $timeout, $location) {
    ...
    var roomListener = function (roomName, otherPeers) {
      /* room listener invoked in self browser when a second person enters */
      console.log('room listner invoked');
      var partnerNameContainer = $window.document.getElementById('partner_name_container');

      /* <div id="partner_name_container"><p>Interview Partner: partnerName</p></div> -> why remove ??? */
      while (partnerNameContainer.hasChildNodes()) {
          console.log('last child removed', partnerNameContainer.lastChild);
      ...
      for (var i in otherPeers) {
        ...
        // wait 3 seconds and if the peer is not connected, call them -> ???? where is it calling the other ???
    ...
    var initRoomListener = function (roomName, otherPeers) {
      console.log('1) initRoomListener entered');
      var partnerNameContainer = $window.document.getElementById('partner_name_container');
      console.log('childNodes: ', partnerNameContainer.hasChildNodes());
      while (partnerNameContainer.hasChildNodes()) {
          partnerNameContainer.removeChild(partnerNameContainer.lastChild);
      }
    ...
    var interviewInit = function () {
      ...
      var initMediaSourceSuccess = function () {
        ...
        /*  getLocalStream: gets a media stream object for the local camera and microphone,
            setVideoObjectSrc: ties a video tag to a media stream object.
        Together they allow a user to monitor how their own image view. */
        ...
        /* roomListener = callback ... find out who else is hooked up to the server */
        $window.easyrtc.setRoomOccupantListener(roomListener);
      };

      var initMediaSourceFailure = function (err) {
        /* console.log(err); // too many errors */
      };
      ...
      // is always possible
      if (currentRoom) {
        /* currentRoom = currentRoom entered */
        $window.easyrtc.getRoomList(function (roomList) {
          /* previous rooms list {numberClients: 0, roomName: 'test'}*/
